...
export default function NewSessionModal({
  ingredientA, ingredientB, allIngredients, onClose,
}: NewSessionModalProps) {
  const router = useRouter()
  const ingredients = allIngredients ?? [ingredientA, ...(ingredientB ? [ingredientB] : [])]

  const [name, setName] = useState(
    ingredients.length <= 2 ? ingredients.join(' + ') : `${ingredientA} + ${ingredients.length - 1} more`
  )
  ...
  return (
    <>
      ...
      <div style={{
        ...
        padding: '24px 24px 40px', maxWidth: 560, margin: '0 auto',
        ...
        <div style={{ width: 40, height: 4, borderRadius: 2, background: 'var(--line-2)', margin: '0 auto 22px' }} />

        <h2 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 24, fontWeight: 700, color: 'var(--ink)', marginBottom: 4 }}>
          Start a Session
        </h2>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 8 }}>
            {categories.map(cat => (
              <button key={cat} onClick={() => setSelectedCat(selectedCat === cat ? '' : cat)} style={{
                ...
                fontSize: 13, fontWeight: selectedCat === cat ? 600 : 400, transition: 'all 0.13s',
              }}>
                {cat}
              </button>
            ))}
            ...
            {addingCat ? (
              ...
              <button onClick={() => setAddingCat(true)} style={{
                padding: '6px 13px', borderRadius: 999, fontFamily: 'inherit', cursor: 'pointer',
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
          ...
          <button onClick={handleCreate} disabled={loading || !name.trim()} style={{
            ...
            background: loading || !name.trim() ? 'var(--line)' : 'var(--green)',

...
UPDATE sessions SET user_id = auth.uid() WHERE user_id IS NULL;
...
CREATE POLICY "canvas_objects: owner all" ON canvas_objects USING (
  EXISTS (SELECT 1 FROM sessions WHERE sessions.id = canvas_objects.session_id AND sessions.user_id = auth.uid())
);

-- canvas_edges
DROP POLICY IF EXISTS "canvas_edges: owner all" ON canvas_edges;
CREATE POLICY "canvas_edges: owner all" ON canvas_edges USING (
  EXISTS (SELECT 1 FROM sessions WHERE sessions.id = canvas_edges.session_id AND sessions.user_id = auth.uid())
);

-- canvas_versions
DROP POLICY IF EXISTS "canvas_versions: owner all" ON canvas_versions;
CREATE POLICY "canvas_versions: owner all" ON canvas_versions USING (
  EXISTS (SELECT 1 FROM sessions WHERE sessions.id = canvas_versions.session_id AND sessions.user_id = auth.uid())
);

-- session_ingredients
DROP POLICY IF EXISTS "session_ingredients: owner all" ON session_ingredients;
CREATE POLICY "session_ingredients: owner all" ON session_ingredients USING (
  EXISTS (SELECT 1 FROM sessions WHERE sessions.id = session_ingredients.session_id AND sessions.user_id = auth.uid())
);
